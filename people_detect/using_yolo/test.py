...
img_color=cv2.imread('videos/people_test4.jpg',cv2.IMREAD_COLOR)
...
blob = cv2.dnn.blobFromImage(img_color, scalefactor=1/255, size=(320, 320), mean=(0, 0, 0), swapRB=True, crop=False)
net.setInput(blob)
...
for output in outputs:
    ...
    for detection in output:
        ...
        if classes[class_id] == "person" and confidence > 0.38:#신뢰도를 조절 할 수 있다.
...
cv2.putText(img_color, 'People Count: {}'.format(people_count), org, font, font_scale, color, thickness, cv2.LINE_AA)
