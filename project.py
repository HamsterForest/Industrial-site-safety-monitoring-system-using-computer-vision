...
#메인루프
def main_loop(btn,allocated,term,auto_range,helmet_check):
...
def second_btn2(allocated, term):
    ...
    button6 = tk.Button(root, text="다음",command= lambda : second_btn3(allocated, term, toggle_var.get()))
    button6.place(x=450, y=300, width=150, height=50)

def second_btn3(allocated, term, auto_range):
    for widget in root.winfo_children():
        widget.destroy()
    button4 = tk.Button(root, text="뒤로 가기", command=bck_btn)
    button4.place(x=30, y=30, width=60, height=30)

    tk.Label(root,text='[안전 헬멧 착용 감지]').place(x=50, y=80)
    tk.Label(root,text='1. 안전 헬멧 착용 감지 기능을 켜는 경우 작업자가 헬멧을 착용했는지 모니터링합니다.').place(x=50, y=110)
    
    tk.Label(root,text='안전 헬멧 착용 감지[ON/OFF]').place(x=50, y=320)

    tk.Label(root, text="ON").place(x=230, y=310)
    toggle_var = tk.IntVar()
    toggle_check = tk.Checkbutton(root,variable=toggle_var)
    toggle_check.place(x=230, y=325)

    tk.Label(root,text='종료시에는 ESC키를 눌러서 종료해야 합니다.').place(x=50, y=360)

    button6 = tk.Button(root, text="모니터링 시작",command= lambda : main_loop(2,allocated,term,auto_range, toggle_var.get()))
    button6.place(x=450, y=300, width=150, height=50)
